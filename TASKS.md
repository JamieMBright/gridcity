...
- **HERO DOCTRINE confirmed:** 12 city modules, EVERY hero carries a bespoke `light:` spec
  (lit-count == key-count per file); **ALL 12/12 at ≥100** (London 100, Paris 105, NYC 100,
  Sydney 103, HK 103, Berlin 100, Shanghai 100, Cairo 100, Athens 100, NE 101, Cape Town 103,
  **Pune 125**). DONE.
  ...
  - ✅ **Pune 31 → 125** (owner 100-MIN doctrine): RESEARCH 33→131 docs + 125 bespoke draws +
    own night lights (Maratha wadas, temple shikharas, chhatris, domed mosques, glass IT towers,
    ghats), all PLACED via named entries in src/data/cities/pune.ts (true-relative geography,
    coincident-placement nudged). Design-gated (preview/pune-heroes-p*.png).
...
- [x] **NE central label NORTHEAST → NEWCASTLE** (#97): MapRenderer.primaryCityLabel
  names the principal city by its biggest town (Newcastle), not the region fabric.
- [x] **NE buildings out of the water** (#97): the de-ponded Tyne was a ~5-tile "lake"
...
- [x] **Inspect a hero → stats + a bit of history**: InfoPanel now surfaces a
  landmark's name + a stat line (type · year · architect/style) + a one-line
  history, from a bundled facts table (src/data/heroFacts.ts) generated by
  tools/buildHeroFacts.ts off docs/heroes/<city>/{index.json,*.md}, linked by the
  PLACED hero name (264 heroes; graceful name-only otherwise). HeroSlot gains a
  `name`. Verified inspecting The Glasshouse / BALTIC (preview/heroc-*.png).
