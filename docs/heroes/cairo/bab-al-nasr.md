# Bab al-Nasr
*fixed construction, city gate, fortified gateway · 1087*

Wikidata: [Q4119350](https://www.wikidata.org/wiki/Q4119350) · Wikipedia: Bab al-Nasr (Cairo)
Reference photo: https://commons.wikimedia.org/wiki/Special:FilePath/Bab%20al-Nasr%20DSCF9983%20(retouched).jpg?width=1000

## Description
gate in the Old City of Cairo, Egypt

Bab al-Nasr, is one of three remaining gates in the historic city wall of Cairo, the capital of Egypt. The gate's construction is dated to 1087 and was ordered by Badr al-Jamali, a Fatimid vizier. It is located at the northern end of Shari'a al-Gamaliya in the old city of Cairo and slightly east of another contemporary gate, Bab al-Futuh.

## Recreation spec (instruction for the bespoke sprite)
Recreate **Bab al-Nasr** as a CUSTOM iso sprite in the game's ink-contour dusk
style — do not reuse another hero. Work from the reference photo above.
- **Type / style:** fixed construction, city gate, fortified gateway.
- **Footprint:** ~1×1 tiles (from its real OSM
  extent of 0.9 tiles). Draw it WIDE within that square.
- **Height:** see description — it must
  TOWER over the ordinary fabric; exaggerate vertically so it reads as a hero.
- **Massing & features:** derive from the description above (towers, domes,
  spires, arches, wings, roofline, materials, colour). Capture the silhouette
  that makes it recognisable.
- **Clearance:** only if the real building has open ground (plaza/park) — most
  abut their neighbours.

Critique the sprite against the reference photo; iterate until the silhouette
and palette read as the real building.
